...
# 3) Crear el Secret con las claves RSA para JWT (ms-auth las monta en /app/keys)
...
- En produccion: gestionar el Secret con SealedSecrets, External Secrets Operator
  o Vault. `secret.example.yaml` y `secret-keys.example.yaml` son solo templates.
- El ConfigMap `krakend-config` se genera automaticamente desde `backend/api-gateway/krakend.json`
  via `configMapGenerator` en el kustomization de api-gateway: cualquier cambio al JSON
  produce un nuevo hash y dispara rolling update del Deployment.
